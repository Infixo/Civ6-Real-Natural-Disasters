...
function AddKeyEntry(textString:string, colorValue:number, bonusIcon:string, bonusValue:string)
	--dprint("FUNCAL AddKeyEntry(textString,colorValue,bonusIcon,bonusValue)",textString,colorValue,bonusIcon,bonusValue);
	...
	--dprint("FUNEND AddKeyEntry(textString)",textString);
end
...
function OnLensLayerOn( layerNum:number )
	...
	elseif layerNum == LensLayers.HEX_COLORING_APPEAL_LEVEL then
		...
		elseif GetCurrentModdedLens() == MODDED_LENS_ID.SCOUT then
			...
			ShowScoutLensKey();
		-- InfixoRND
		elseif GetCurrentModdedLens() == MODDED_LENS_ID.DISASTER_EVENT then
			Controls.LensText:SetText(Locale.ToUpper(Locale.Lookup("LOC_HUD_DISASTER_LENS")));
			ShowDisasterEventLensKey();
		elseif GetCurrentModdedLens() == MODDED_LENS_ID.DISASTER_RISK_ET then
			Controls.LensText:SetText(Locale.ToUpper(Locale.Lookup("LOC_HUD_DISASTER_ET_LENS")));
			ShowDisasterRiskETLensKey();
		elseif GetCurrentModdedLens() == MODDED_LENS_ID.DISASTER_RISK_FT then
			Controls.LensText:SetText(Locale.ToUpper(Locale.Lookup("LOC_HUD_DISASTER_FT_LENS")));
			ShowDisasterRiskFTLensKey();
		elseif GetCurrentModdedLens() == MODDED_LENS_ID.DISASTER_RISK_VW then
			Controls.LensText:SetText(Locale.ToUpper(Locale.Lookup("LOC_HUD_DISASTER_VW_LENS")));
			ShowDisasterRiskVWLensKey();
		end
...
print("Finished loading ModalLensPanel.lua from Real Natural Disasters");