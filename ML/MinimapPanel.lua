print("Loading MinimapPanel.lua from Real Natural Disasters");
-- ===========================================================================
--  MINIMAP PANEL
-- ===========================================================================
...
-- Author: Infixo
-- DateCreated: 3/28/2017 12:07:27 AM
...
local MODDED_LENS_ID:table = {
  ...
  SCOUT = 10;
	-- InfixoRND
	DISASTER_EVENT		= 11;  -- recent event
	DISASTER_RISK_ET	= 12;  -- Earthquake, Tornado
	DISASTER_RISK_FT 	= 13;  -- Flood, Tsunami
	DISASTER_RISK_VW 	= 14;  -- Volcano, Wildfire
};
...
function OnToggleLensList()
  ...
  if Controls.LensPanel:IsHidden() then
	...
	-- InfixoRND
    Controls.DisasterEventLensButton:SetCheck(false);
    Controls.DisasterETLensButton:SetCheck(false);
    Controls.DisasterFTLensButton:SetCheck(false);
    Controls.DisasterVWLensButton:SetCheck(false);
...
function ToggleAnyModdedLens(pControlButton:table, eLensToToggle:number)
	...
	--dprint("FUNEND ToggleAnyModdedLens(eLensToToggle)", eLensToToggle);
end
function ToggleDisasterEventLens()
	ToggleAnyModdedLens(Controls.DisasterEventLensButton, MODDED_LENS_ID.DISASTER_EVENT);
end
function ToggleDisasterETLens()
	ToggleAnyModdedLens(Controls.DisasterETLensButton, MODDED_LENS_ID.DISASTER_RISK_ET);
end
function ToggleDisasterFTLens()
	ToggleAnyModdedLens(Controls.DisasterFTLensButton, MODDED_LENS_ID.DISASTER_RISK_FT);
end
function ToggleDisasterVWLens()
	ToggleAnyModdedLens(Controls.DisasterVWLensButton, MODDED_LENS_ID.DISASTER_RISK_VW);
end
...
function OnLensLayerOn( layerNum:number )
  ...
  elseif layerNum == LensLayers.HEX_COLORING_APPEAL_LEVEL then
    ...
    elseif currentModdedLens == MODDED_LENS_ID.SCOUT then
      SetScoutLens();
	-- InfixoRND
    elseif currentModdedLens == MODDED_LENS_ID.DISASTER_EVENT then
      SetDisasterEventHexes();
    elseif currentModdedLens == MODDED_LENS_ID.DISASTER_RISK_ET then
      SetDisasterETHexes();
    elseif currentModdedLens == MODDED_LENS_ID.DISASTER_RISK_FT then
      SetDisasterFTHexes();
    elseif currentModdedLens == MODDED_LENS_ID.DISASTER_RISK_VW then
      SetDisasterVWHexes();
    end
    UI.PlaySound("UI_Lens_Overlay_On");
...
-- ===========================================================================
-- InfixoRND Functions to actually show hexes
...
function Initialize()
  ...
  Controls.BuilderLensButton:RegisterCallback( Mouse.eLClick, ToggleBuilderLens );
	...
	-- InfixoRND
    Controls.DisasterEventLensButton:RegisterCallback( Mouse.eLClick, ToggleDisasterEventLens );
    Controls.DisasterETLensButton:RegisterCallback( Mouse.eLClick, ToggleDisasterETLens );
    Controls.DisasterFTLensButton:RegisterCallback( Mouse.eLClick, ToggleDisasterFTLens );
    Controls.DisasterVWLensButton:RegisterCallback( Mouse.eLClick, ToggleDisasterVWLens );
...
print("Finished loading MinimapPanel.lua from Real Natural Disasters");